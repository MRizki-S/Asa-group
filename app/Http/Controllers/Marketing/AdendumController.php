...
class AdendumController extends Controller
{
    ...
    protected function handleCaraBayar(Request $request)
    {
        ...
        try {
            ...
            // 🔹 Update pemesanan_unit terkait cara_bayar baru
            if ($caraBayarLama === $request->cara_bayar_baru) {
                // Cara bayar sama → update data yang relevan
                if ($request->cara_bayar_baru === 'kpr') {
                    // dd('masuk sini kpr');
                    $pemesananUnit->kpr()->update([
                        'dp_rumah_induk'        => $request->kpr_dp_rumah_induk_baru,
                        'dp_dibayarkan_pembeli' => $request->kpr_dp_dibayarkan_pembeli_baru,
                        'sbum_dari_pemerintah'  => $request->kpr_sbum_dari_pemerintah_baru ?? 0,
                        'luas_kelebihan'        => $request->kpr_luas_kelebihan_baru ?? null,
                        'nominal_kelebihan'     => $request->kpr_nominal_kelebihan_baru ?? null,
                        'total_dp'              => $request->kpr_total_dp_baru,
                        'harga_kpr'             => $request->kpr_harga_kpr_baru,
                        'harga_total'           => $request->kpr_harga_total_baru,
                    ]);
                } else { // cash
                    $pemesananUnit->cash()->update([
                        'harga_rumah'       => $request->cash_harga_rumah,
                        'luas_kelebihan'    => $request->cash_luas_kelebihan ?? null,
                        'nominal_kelebihan' => $request->cash_nominal_kelebihan ?? null,
                        'harga_jadi'        => $request->cash_harga_jadi,
                ...
                // dd('berhasil update cara bayar sama');
            } else {
                // 🔹 Cara bayar berubah → hapus table lama
                if ($caraBayarLama === 'kpr') {
                    $pemesananUnit->kpr()->delete();
                    // dd('berhasil hapus kpr lama');
                } else {
                    $pemesananUnit->cash()->delete();
                }

                // 🔹 Insert ke table baru sesuai cara_bayar_baru
                if ($request->cara_bayar_baru === 'kpr') {
                    $pemesananUnit->kpr()->create([
                        'dp_rumah_induk'        => $request->kpr_dp_rumah_induk_baru,
                        'dp_dibayarkan_pembeli' => $request->kpr_dp_dibayarkan_pembeli_baru,
                        'sbum_dari_pemerintah'  => $request->kpr_sbum_dari_pemerintah_baru ?? 0,
                        'luas_kelebihan'        => $request->kpr_luas_kelebihan_baru ?? null,
            ...
            // 🔹 Nonaktifkan cicilan lama yang belum lunas
            PemesananUnitCicilan::where('pemesanan_unit_id', $pemesananUnit->id)
                ->where('status_bayar', '!=', 'lunas')
                ->update(['is_active' => 0]);

            // 🔹 Insert cicilan baru (hanya yang belum lunas)
