...
class PembangunanUnitController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = PembangunanUnit::with([
            'perumahaan:id,nama_perumahaan,slug',
            'tahap:id,perumahaan_id,nama_tahap,slug',
            'unit:id,blok_id,nama_unit',
            'pengawas:id,nama_lengkap',
            'qcContainer',
            'pengajuan'
        ])->where('perumahaan_id', $perumahaanId)->whereIn('status_pembangunan',  ['proses', 'selesai'])->latest('created_at');
        ...
        $allPembangunanUnit = $query->get()->map(function($unit) {
            ...
            foreach ($unit->pembangunanUnitQc as $qc) {
                $qcProgress = $qc->total_task > 0
                    ? ($qc->task_selesai_count / $qc->total_task) * 100
                    : 0;
            ...
            $unit->total_progres = $totalQc > 0
                ? round($sumProgressQc / $totalQc, 2)
                : 0;
    ...
    public function sendAcceptNotification($pembangunan)
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('produksi.pembangunanUnit.index')
            ->with('success', 'Data Pengajuan Pembangunan Unit berhasil diassign!');
...
{
    $data = PembangunanUnit::with([
        'unit', 'tahap', 'perumahaan', 'pengawas',
        'pembangunanUnitQc.pembangunanUnitQcTask',
    ...
    return view('Produksi.pembangunan-unit.show', [
        ...
        'breadcrumbs' => [
            ['label' => 'Pembangunan Unit', 'url' => route('produksi.pembangunanUnit.index')],
            ['label' => 'Detail ' . $data->unit->nama_unit, 'url' => '#'],
        ],
    ]);
}
