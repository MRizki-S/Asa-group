...
@section('pageActive', 'managerPersetujuanUpahKontraktor')
...
@section('content')
    <div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6" x-data="{
        ...
        confirmAction(type) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.action = `/manager/persetujuan-upah-kontraktor/${this.selectedItem.id}`;
        ...
        <div x-data="{ pageName: 'Persetujuan Upah Pemb. Proyek' }">
            ...
            <div
                ...
                <div class="mb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <h3 class="text-base font-medium text-gray-800 dark:text-white/90">
                        Daftar Pengajuan Upah Pemb. Proyek
                    </h3>

                    {{-- Filter Dropdown --}}
                    <form action="{{ route('manager.persetujuanUpahKontraktor.index') }}" method="GET" id="form-filter">
                                    ...
                                    <td class="px-4 py-4 text-center">
                                        ...
                                        @if (!$isFinal)
                                            <button type="button"
                                                @click="openModal({
                                                        id: '{{ $item->id }}',
                                                        unit_nama: '{{ $item->proyek->nama ?? '-' }}',
                                                        upah_nama: '{{ $item->nama_upah }}',
                                                        pengawas: '{{ $item->proyek->pengawas->nama_lengkap ?? '-' }}',
