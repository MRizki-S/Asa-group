...
@section('pageActive', 'managerPersetujuanUpahKawasan')
...
@section('content')
    <div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6" x-data="{
        ...
        confirmAction(type) {
            ...
            form.action = `/manager/persetujuan-upah-kawasan/${this.selectedItem.id}`;
        ...
        <div x-data="{ pageName: 'Persetujuan Upah Pemb. Kawasan' }">
            ...
            <div
                ...
                <div class="mb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <h3 class="text-base font-medium text-gray-800 dark:text-white/90">
                        Daftar Pengajuan Upah Pemb. Kawasan
                    </h3>

                    {{-- Filter Dropdown --}}
                    <form action="{{ route('manager.persetujuanUpahKawasan.index') }}" method="GET" id="form-filter">
                ...
                <div class="max-w-full overflow-x-auto custom-scrollbar">
                    <table id="table-upah" class="min-w-full" style="min-width: 1000px;">
                        <thead>
                            <tr>
                                <th class="bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-400">Kawasan / Pekerjaan
                                </th>
                                <th class="bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-400">Tanggal Pengajuan</th>
                            ...
                            @foreach ($allUpahPengajuan as $item)
                                <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
                                    ...
                                    <td class="px-4 py-4 uppercase">
                                        <div class="text-[10px]">
                                            <p class="font-semibold text-gray-800 dark:text-gray-200 text-xs tracking-tight">
                                    ...
                                    <td class="px-4 py-4 text-center">
                                        @php
                                            $isFinal =
                                                ...
                                                $item->disetujui_mgr_dukungan !== null;
                                        @endphp
                                        ...
                                        @if (!$isFinal)
                                            <button type="button"
                                                @click="openModal({
                                                        id: '{{ $item->id }}',
                                                        unit_nama: '{{ $item->kawasan->nama ?? '-' }}',
                                                        upah_nama: '{{ $item->nama_upah }}',
                                                        pengawas: '{{ $item->kawasan->pengawas->nama_lengkap ?? '-' }}',
                                        ...
                                        @else
                                            <div class="flex flex-col items-center justify-center gap-1">
                                                <div class="flex flex-col items-center justify-center gap-1">
                                                    @if ($item->status_pengajuan === 'ditolak_mgr_dukungan')
                                                        <span
