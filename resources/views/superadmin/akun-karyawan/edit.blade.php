    ...
    <div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6" x-data="{ showPassword: false }">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-800">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">Perbarui data karyawan <strong>{{ $user->nama_lengkap }}</strong>.</p>
            </div>

            <form action="{{ route('superadmin.akunKaryawan.update', $user->id) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    ...
                    <div>
                        ...
                        @php
                            $noHpDisplay = $user->no_hp;
                            if (str_starts_with($noHpDisplay, '62')) {
                                $noHpDisplay = substr($noHpDisplay, 2);
                            }
                        @endphp
                        <div class="flex">
                            ...
                            <input type="number" name="no_hp" required value="{{ old('no_hp', $noHpDisplay) }}" placeholder="8123xxxx"
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-semibold text-gray-700 dark:text-white">
                            Password Login <span class="text-gray-400 font-normal">(Kosongkan jika tidak ingin mengubah)</span>
                        </label>
                        <div class="relative">
                            <span class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-gray-400">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div>
                        ...
                        @php
                            $currentUbs = $user->is_global ? 'HUB' : $user->perumahaan_id;
                        @endphp
                        <select name="perumahaan_id" required class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-blue-500 block p-2.5 dark:bg-gray-800 dark:border-gray-700 dark:text-white">
                            ...
                            <option value="HUB" {{ old('perumahaan_id', $currentUbs) == 'HUB' ? 'selected' : '' }}>HUB (PUSAT)</option>
                            @foreach ($ubs as $u)
                                @php
                                ...
                                <option value="{{ $u->id }}" 
                                    {{ $isMangoon ? 'disabled' : '' }}
                                    {{ old('perumahaan_id', $currentUbs) == $u->id ? 'selected' : '' }}>
