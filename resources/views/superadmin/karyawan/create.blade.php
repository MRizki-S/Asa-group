...
@section('content')
    ...
    <div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <form action="{{ route('superadmin.karyawan.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    ...
                    <div>
                        ...
                        <select name="ubs_id" required class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-blue-500 block p-2.5 dark:bg-gray-800 dark:border-gray-700 dark:text-white">
                            ...
                            <option value="HUB" {{ old('ubs_id') == 'HUB' ? 'selected' : '' }}>HUB (PUSAT)</option>
                            @foreach ($ubs as $u)
                                <option value="{{ $u->id }}" 
                                    {{ old('ubs_id') == $u->id ? 'selected' : '' }}>
                                    {{ $u->nama_ubs }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <p class="mt-1.5 text-[10px] text-gray-400 italic font-medium">*Daftar user di atas adalah user login bertipe karyawan yang belum dikaitkan dengan karyawan manapun (Bisa memilih lebih dari satu akun).</p>
                    </div>
                </div>
