...
@section('content')
    <div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6">
        ...
        <div class="mb-4 flex justify-end">
            <a href="{{ $type === 'cash' ? route('settingPPJB.promoCash.edit') : route('settingPPJB.promoKpr.edit') }}"
                            ...
                            <div
                                ...
                                <div class="flex items-center justify-between mb-2">
                                    <h4 class="font-semibold text-gray-800 dark:text-gray-200 text-lg">
                                        {{ $batch->judul ?? 'Promo Cash' }} – Non-Aktif
                                    </h4>
                                    <span
                                ...
                                <div class="mt-3 text-sm text-gray-500 dark:text-gray-400">
                                    Disetujui oleh
                                    <strong>{{ $batch->penyetuju->username ?? '-' }}</strong>
                                    &middot; Diajukan
                                    {{ $batch->tanggal_pengajuan?->format('d M Y') ?? '-' }}
                                    &middot; Nonaktif sejak
                            ...
                            <div
                                ...
                                <div class="flex items-center justify-between mb-2">
                                    <h4 class="font-semibold text-red-800 dark:text-red-300 text-lg">
                                        {{ $batch->judul ?? 'Promo Cash' }} – Ditolak
                                    </h4>
                                    <span
                                ...
                                {{-- List Items --}}
                                <ul class="list-decimal list-inside space-y-1 text-red-900 dark:text-red-200 text-sm">
                                    @foreach ($batch->items as $item)
                                        <li>{{ $item->nama_promo }}</li>
                                    @endforeach
                                </ul>
                                ...
                                <div class="mt-3 text-sm text-red-700 dark:text-red-300">
                                    Diajukan oleh
                                    <strong>{{ $batch->pengaju->username ?? '-' }}</strong>
                                    pada {{ $batch->tanggal_pengajuan?->format('d M Y') ?? '-' }}
                                    &middot; Ditolak pada
                                    {{ $batch->updated_at->format('d M Y') }}
                                    @if (!empty($batch->alasan_tolak))
                                        <br><span class="italic">Alasan: {{ $batch->alasan_tolak }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
