...
@section('content')
        ...
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] mb-6">
            <div class="px-5 py-4 sm:px-6 sm:py-5">
                ...
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-base font-medium text-gray-800 dark:text-white/90">Bonus KPR PPJB</h3>

                    <div class="flex items-center gap-2">
                        {{-- Tombol Riwayat Bonus KPR --}}
                        <a
                        href="{{ route('settingPPJB.bonusKpr.history') }}"
                            class="px-4 py-2 bg-gray-200 text-gray-800 text-sm rounded-md hover:bg-gray-300
                         dark:bg-gray-700 dark:text-white dark:hover:bg-gray-600">
                            Riwayat Bonus KPR PPJB
                        </a>
                ...
                @if (!$bonusKprActive && !$bonusKprPending)
                    <div class="mb-4">
                        <div
                            class="rounded-xl border border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800/20 p-4 text-center">
                            <p class="text-sm text-gray-600 dark:text-gray-300">Belum ada Bonus Kpr PPJB aktif saat ini.
                            </p>
                        </div>
                    </div>
                @endif
                    ...
                    <div class="mb-4">
                        <div
                            ...
                            <div class="flex items-center justify-between mb-2">
                                <h4 class="font-semibold text-green-800 dark:text-green-300 text-lg">Bonus KPR PPJB (Aktif)
                                </h4>
                            ...
                            <ul class="space-y-3 mb-4">
                                @if ($bonusKprActive->items->count())
                                    @foreach ($bonusKprActive->items as $item)
                                        <li class="flex items-center">
                                            <span class="text-gray-800 dark:text-gray-200">{{ $item->nama_bonus }}</span>
                                        </li>
                                    @endforeach
                                @else
                                    <li class="text-gray-500 dark:text-gray-400 italic">Belum ada bonus aktif.</li>
                                @endif
                            </ul>
                            ...
                            <div class="flex justify-between items-center mt-2">
                                <p class="text-sm text-gray-500">
                                    Disetujui oleh <strong>{{ $bonusKprActive->penyetuju->username ?? '-' }}</strong>
                                    pada
                                    {{ $bonusKprActive->tanggal_acc ? $bonusKprActive->tanggal_acc->translatedFormat('d M Y') : '-' }}
                                </p>
                                <div class="mt-4 flex justify-end">

                                    @can('marketing.setting-ppjb.kelola.nonaktif')
                                    <form action="{{ route('settingPPJB.bonusKpr.nonAktif', $bonusKprActive) }}"
                                        method="POST" class="delete-form">
                                        @csrf
                                        @method('PATCH')
                                        <button type="button"
                                            class="nonAktifkanBonusKpr KPR px-3 py-1 text-sm bg-red-600 text-white rounded-md hover:bg-red-700">
                                            Nonaktifkan
                                        </button>
                                    </form>
                                    @endcan
                ...
                @if ($bonusKprPending)
                    <div class="mb-4">
                        <div
                            ...
                            {{-- Header --}}
                            <div class="flex items-center justify-between mb-2">
                                <h4 class="font-semibold text-yellow-800 dark:text-yellow-300 text-lg">Bonus KPR PPJB -
                                    Diajukan
                                    (Pending)</h4>
                            ...
                            <ul class="list-decimal list-inside space-y-1 text-gray-700 dark:text-gray-300 text-sm">
                                @foreach ($bonusKprPending->items as $item)
                                    <li>{{ $item->nama_bonus }}
                                    </li>
                                @endforeach
                            </ul>
                            ...
                            <div class="flex justify-between items-center">
                                <p class="text-sm text-gray-500 mt-2">
                                    Diajukan oleh <strong>{{ $bonusKprPending->pengaju->username ?? '-' }}</strong>
                                    pada {{ $bonusKprPending->tanggal_pengajuan->format('d M Y') }}
                                </p>
                                <div class="flex gap-2 sm:justify-end justify-start">
                                    @can('marketing.setting-ppjb.kelola.action')
                                        {{-- Tombol Tolak --}}
                                        <form action="{{ route('settingPPJB.bonusKpr.reject', $bonusKprPending) }}"
                                            method="POST" class="delete-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
                                        ...
                                        {{-- Tombol ACC --}}
                                        <form
                                            action="{{ route('settingPPJB.bonusKpr.approve', $bonusKprPending) }}" method="POST"
                                            class="approve-form">
                                            @csrf
                                            @method('PATCH')
                                            <button type="button"
                                    ...
                                    @can('marketing.setting-ppjb.kelola.cancel')
                                        <form action="{{ route('settingPPJB.bonusKpr.cancel', $bonusKprPending) }}"
                                            method="POST" class="delete-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
    ...
    {{-- Modal Ajukan Bonus KPR PPJB Baru --}}
        ...
        <div class="relative w-full max-w-xl p-4">
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700 flex flex-col max-h-[90vh]">
                ...
                <div class="overflow-y-auto px-4 py-6 space-y-4 flex-1">
                    <form id="bonusKprForm" action="{{ route('settingPPJB.bonusKpr.pengajuanUpdate') }}" method="POST"
                        class="space-y-4">
                        @csrf
                        ...
                        {{-- Dynamic input list --}}
                        <div id="bonusKpr-list" class="space-y-3">
                            <div class="flex items-center gap-2 bonusKpr-item">
                                {{-- Nama Bonus KPR --}}
                        ...
                        {{-- Tombol tambah --}}
                        <div>
                            <button type="button" id="addBonusKpr"
                ...
                <div class="flex justify-end gap-3 p-4 border-t dark:border-gray-600">
                    ...
                    <button type="submit" form="bonusKprForm"
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            const list = document.getElementById("bonusKpr-list");
            const addRow = document.getElementById("addBonusKpr");

            // Tambah row baru
            addRow.addEventListener("click", () => {
                const div = document.createElement("div");
                div.classList.add("flex", "items-center", "gap-2", "bonusKpr-item");
                div.innerHTML = `
                <div class="flex-1 flex flex-col">
                    <label class="text-sm text-gray-700 dark:text-gray-300 mb-1">
                        Nama Bonus KPR
            ...
            list.addEventListener("click", (e) => {
                if (e.target.classList.contains("remove-btn")) {
                    e.target.closest(".bonusKpr-item").remove();
                }
            });
        });
        ...
        // {{-- sweatalert 2 for batalkan pengajuan Bonus KPR data --}}
        document.addEventListener('click', function(e) {
            if (e.target.closest('.cancelPengajuan')) {
                ...
                Swal.fire({
                    title: 'Yakin membatalkan pengajuan Bonus KPR ini?',
                    icon: 'warning',
        ...
        // {{-- sweatalert 2 for nonaktifkan Bonus kpr data --}}
        document.addEventListener('click', function(e) {
            if (e.target.closest('.nonAktifkanBonusKpr')) {
                const btn = e.target.closest('.nonAktifkanBonusKpr');
                const form = btn.closest('.delete-form');

                Swal.fire({
                    title: 'Yakin menonaktifkan Bonus KPR?',
                    text: 'Menonaktifkan Bonus KPR akan berpengaruh pada proses PPJB dan semua data terkait batch ini. Pastikan Anda ingin melanjutkan.',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#d33',
                    cancelButtonColor: '#3085d6',
        ...
        // 🛑 SweetAlert untuk Tolak Pengajuan (Bonus KPR)
        document.addEventListener('click', function(e) {
            if (e.target.closest('.tolakPengajuan')) {
                ...
                Swal.fire({
                    title: 'Tolak Pengajuan Bonus KPR?',
                    text: 'Apakah Anda yakin ingin menolak pengajuan Bonus KPR ini? Tindakan ini tidak dapat dibatalkan.',
                    icon: 'warning',
                    showCancelButton: true,
        ...
        // ✅ SweetAlert untuk ACC Pengajuan Bonus KPR
        document.addEventListener('click', function(e) {
            if (e.target.closest('.accPengajuan')) {
                ...
                Swal.fire({
                    title: 'Setujui Pengajuan Bonus KPR ini?',
                    text: 'Hanya satu pengajuan Bonus KPR yang bisa aktif. Jika disetujui, Bonus KPR aktif sebelumnya akan dinonaktifkan dan digantikan dengan pengajuan ini.',
                    icon: 'question',
                    showCancelButton: true,
                    cancelButtonColor: '#6b7280',
