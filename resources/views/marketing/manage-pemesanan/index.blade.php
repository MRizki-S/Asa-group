...
@section('content')
    <!-- ===== Main Content Start ===== -->
    <div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6" x-data="{ showExportModal: false, exportSource: 'internal', exportTitle: 'Export Data Closing (Internal)' }">
        ...
        @php
            ...
            if ($namaPerumahaanAktif === 'Asa Dreamland') {
                // Khusus ADL → ROLE
                                $bolehPrintPPJB = $user->hasRole(['PROYEK MANAJER (ADL)', 'STAF PENJUALAN (ADL)', 'superadmin', 'STAF SOLUSI KPR & PENJUALAN (ADL)']);
            } else {
        ...
        <div class="space-y-6">
            ...
            <form method="GET" action="{{ route('marketing.managePemesanan.index') }}"
                ...
                <div class="flex gap-2">
                    ...
                    @endif
                    @can('marketing.kelola-pemesanan.export-closing')
                        <div class="relative" x-data="{ openExportDropdown: false }" @click.away="openExportDropdown = false">
                            ...
                            <div x-show="openExportDropdown" x-cloak
                                ...
                                <button type="button" @click="exportSource = 'internal'; exportTitle = 'Export Data Closing (Internal)'; showExportModal = true; openExportDropdown = false"
                                    ...
                                    <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 group-hover:bg-blue-100 dark:group-hover:bg-blue-900/50 transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                        </svg>
                                    </div>
                                    <div class="text-left">
                                        <div class="font-medium text-gray-900 dark:text-white">Export Internal (Sales)</div>
                                        <div class="text-[11px] text-gray-400 font-normal">Transaksi sales in-house</div>
                                    </div>
                                </button>
    ...
    <div x-show="showExportModal" class="fixed inset-0 z-[99999] flex items-center justify-center bg-black/50" x-cloak>
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-4xl p-6 border border-gray-200 dark:border-gray-700"
            ...
            <form action="{{ route('marketing.managePemesanan.exportExcel') }}" method="GET" @submit="showExportModal = false">
