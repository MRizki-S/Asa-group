...
@section('content')
    ...
    <div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6">
        ...
        <div x-data="{ openInfo: false, openCash: false, openKpr: false }" class="mt-6 mb-6 rounded-xl border border-gray-200 dark:border-gray-700
            ...
            <div x-show="openInfo" x-transition x-cloak class="px-5 py-5">
                ...
                @if ($promoCash)
                    <div class="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                        <button @click="openCash = !openCash" class="w-full flex justify-between items-center px-4 py-2.5
                        ...
                        <div x-show="openCash" x-transition x-cloak class="p-4 bg-white dark:bg-gray-800 text-sm leading-snug">
                ...
                @if ($promoKpr)
                    <div class="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden mt-3">
                        <button @click="openKpr = !openKpr" class="w-full flex justify-between items-center px-4 py-2.5
                                   bg-gray-50 dark:bg-gray-700/40 text-sm font-medium
                                   hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                        ...
                        <div x-show="openKpr" x-transition x-cloak class="p-4 bg-white dark:bg-gray-800 text-sm leading-snug">
                            <ul class="list-disc ml-5 space-y-1">
        ...
        <form action="{{ route('marketing.pemesananUnit.store') }}" method="POST" x-data="bookingForm()">
            ...
            {{-- Sub 1.5: Pilih Fee Agen (Hanya SPV Pembiayaan KPR) --}}
            @if ($isSPVKPR)
                <div x-show="hasSelected && selectedCustomer.booking.source === 'agent'" x-transition.opacity
                    ...
                    <div class="px-5 py-4 sm:px-6 sm:py-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Pilih Skema Fee
                                    Agen</label>
                                <select name="master_agent_fee_id"
                                    ...
                                    @foreach ($feeAgens as $fee)
                                        <option value="{{ $fee->id }}">
                                            {{ $fee->judul_fee }} - (Rp {{ number_format($fee->nominal, 0, ',', '.') }})
                                        </option>
                                    @endforeach
                                </select>
                                <p class="mt-1 text-xs text-gray-500 italic">* Pilih skema fee yang akan diberikan kepada agen
                                    untuk pemesanan ini.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            @can('marketing.pemesanan-unit.create')
                <div class="flex justify-end gap-2">
                    ...
                    <button type="submit" class="px-8 py-2.5 text-sm font-medium text-white rounded-lg bg-blue-600 hover:bg-blue-700
