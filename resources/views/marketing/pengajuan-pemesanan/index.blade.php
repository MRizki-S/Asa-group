...
@section('content')
            ...
            @if ($isGlobal)
                @if ($pengajuanPemesanan->isEmpty())
                   ...
                   dark:border-gray-800 dark:bg-white/[0.03]">
                        Tidak ada data pengajuan pemesanan unit.
                    </div>
                @endif
                @foreach ($pengajuanPemesanan as $perumahaanId => $items)
                    @php
                        $namaPerumahaan = $items->first()->perumahaan->nama_perumahaan ?? 'Tidak Diketahui';
                    @endphp
    ...
    <script>
        if (document.getElementById("table-pengajuanPemesanan") && typeof simpleDatatables.DataTable !== 'undefined') {
            const dataTable = new simpleDatatables.DataTable("#table-pengajuanPemesanan", {
                searchable: true,
                sortable: true,
            });
        }
