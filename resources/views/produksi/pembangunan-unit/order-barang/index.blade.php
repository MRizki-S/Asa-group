...
<div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6">
    <div x-data="{ pageName: 'Order Barang Unit' }">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-gray-100 dark:border-gray-800 pb-4">
            <div>
                <h3 class="text-lg font-bold text-gray-800 dark:text-white">Daftar Order Barang Unit</h3>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Riwayat dan daftar pengajuan order barang oleh pengawas proyek</p>
            </div>
            <div>
                @can('produksi.properti.pembangunan-unit.order-barang.create')
                <a href="{{ route('produksi.pembangunanUnit.orderCreate') }}"
        ...
        <div class="mb-4 flex flex-wrap items-center gap-2">
            ...
            <a href="{{ route('produksi.pembangunanUnit.orderIndex', ['status' => 'selesai']) }}"
            ...
            <a href="{{ route('produksi.pembangunanUnit.orderIndex', ['status' => 'ditolak']) }}"
            ...
            <table class="w-full text-left text-sm text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
                <thead class="bg-gray-50 text-xs uppercase text-gray-600 dark:bg-gray-800 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
                    <tr>
                        ...
                        <th class="px-4 py-3">Perumahan / Unit</th>
                        <th class="px-4 py-3">QC RAP</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse ($orders as $order)
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                            ...
                            <td class="px-4 py-3">
                                @if ($order->pembangunan_unit_id)
                                    <a href="{{ route('produksi.pembangunanUnit.show', $order->pembangunan_unit_id) }}" class="group block">
                            ...
                            <td class="px-4 py-3 text-xs">
                                @if ($order->pembangunan_unit_id && $order->pembangunan_unit_qc_id)
                                    <a href="{{ route('produksi.pembangunanUnit.show', $order->pembangunan_unit_id) }}?qc={{ $order->pembangunan_unit_qc_id }}&tab=bahan"
                                        ...
                                        title="Buka detail unit pada Tab Bahan QC ini">
                                        <span>{{ $order->qc?->nama_qc ?? '-' }}</span>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    @if ($order->status_order === 'selesai')
                                        <a href="{{ route('gudang.permintaanBarang.pembangunanUnit.notaPdf', $order->id) }}"
                                            target="_blank"
                                    ...
                                    @if ($order->status_order === 'diproses')
                                        @can('produksi.properti.pembangunan-unit.order-barang.delete')
                                        <form method="POST" action="{{ route('produksi.pembangunanUnit.orderDestroy', $order->id) }}" class="form-batal-order">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-8 text-center text-sm text-gray-500">
                                Belum ada data order barang unit.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
