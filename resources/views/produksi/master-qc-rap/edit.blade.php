...
@section('content')
    ...
    <div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6" x-data="qcEditForm(
        @js(
    $container->urutan->map(
        fn($u) => [
            'qc_ke' => $u->qc_ke,
            'nama_qc' => $u->nama_qc,
                            ...
                            <select name="type_id" required id="selectTypeUnit"
                                ...
                                @foreach ($allType ?? [] as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $container->type_id == $item->id ? 'selected' : '' }}>
                                        {{ $item->nama_type }}
                                    </option>
                                @endforeach
                            </select>
                ...
                removeQc(index) {
                    Swal.fire({
                        ...
                        if (r.isConfirmed) {
                            ...
                            this.bahanGroups.forEach(b => {
                                if (b.urutan_idx > index) b.urutan_idx--;
                            });
                            this.upahGroups.forEach(u => {
                                if (u.urutan_idx > index) u.urutan_idx--;
                            });
                        }
                    });
                },
