...
@section('content')
    <div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6"
         x-data="upahManager({{ $allMasterUpah->map(fn($u) => [
            'id' => $u->id,
        ...
        <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <button @click="openModal('create')" class="inline-flex items-center justify-center px-5 py-2.5 text-sm font-bold text-white bg-blue-600 rounded-lg hover:bg-blue-700 shadow-lg">
                    ...
                    <div class="flex flex-col h-full justify-between">
                        <div>
                            <div class="flex items-center justify-between mb-3">
                                <span class="rounded-lg bg-blue-50 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-blue-600 dark:bg-blue-900/30 dark:text-blue-400" x-text="'ID: ' + upah.id"></span>
                                <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button @click="openModal('edit', upah)" class="p-2 text-gray-400 hover:text-blue-600 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                    </button>
                                    <button @click="confirmDelete(upah.id)" class="p-2 text-gray-400 hover:text-red-600 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                    </button>
                                </div>
                            </div>
                            <h4 class="text-lg font-bold text-gray-800 dark:text-white leading-snug" x-text="upah.nama_upah"></h4>
                        </div>
                        <div class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 flex items-center text-xs text-gray-500 dark:text-gray-400">
                            <svg class="w-4 h-4 me-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
        ...
        <template x-for="upah in allData" :key="'form-'+upah.id">
            <form :class="'delete-form-'+upah.id" :action="'/produksi/master-upah/'+upah.id" method="POST" style="display:none;">
                @csrf @method('DELETE')
            </form>
        </template>
    </div>
        ...
        function upahManager(initialData) {
            return {
                ...
                openModal(mode, data = null) {
                    this.modalMode = mode;
                    if (mode === 'edit') {
                        this.formData = { id: data.id, nama: data.nama_upah };
