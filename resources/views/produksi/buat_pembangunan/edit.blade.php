        ...
        <form action="{{ route('produksi.buatPembangunanKawasan.update', $kawasan->id) }}" method="POST">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <div class="relative" x-data="{ simpan: '{{ $kawasan->tanggal_mulai ? \Carbon\Carbon::parse($kawasan->tanggal_mulai)->format('Y-m-d') : '' }}' }">
                ...
                <div>
                    ...
                    <div class="relative" x-data="{ simpan: '{{ $kawasan->tanggal_selesai ? \Carbon\Carbon::parse($kawasan->tanggal_selesai)->format('Y-m-d') : '' }}' }">
                        <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none z-10">
                            <svg class="w-4 h-4 text-gray-400" fill="currentColor" viewBox="0 0 20 20"><path d="M20 4a2 2 0 0 0-2-2h-2V1a1 1 0 0 0-2 0v1h-3V1a1 1 0 0 0-2 0v1H6V1a1 1 0 0 0-2 0v1H2a2 2 0 0 0-2 2v2h20V4ZM0 18a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8H0v10Zm5-8h10a1 1 0 0 1 0 2H5a1 1 0 0 1 0-2Z" /></svg>
                        </div>
                        <input type="text" x-init="flatpickr($el, { dateFormat: 'd-m-Y', defaultDate: '{{ $kawasan->tanggal_selesai ? \Carbon\Carbon::parse($kawasan->tanggal_selesai)->format('d-m-Y') : '' }}', onChange: (selectedDates, dateStr, instance) => { simpan = instance.formatDate(selectedDates[0], 'Y-m-d'); } })" class="w-full pl-10 pr-3 py-2 text-gray-700 rounded-lg border border-gray-300 bg-gray-50 text-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white outline-none" placeholder="Pilih Tanggal Selesai">
                        <input type="hidden" name="tanggal_selesai" x-model="simpan">
                    </div>
                </div>

                <div class="flex gap-2 pt-2">
                    <button type="submit" class="flex-1 rounded-lg bg-blue-700 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Update Pembangunan</button>
                    <a href="{{ route('produksi.buatPembangunanKawasan.index') }}" class="flex-1 rounded-lg bg-gray-500 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-gray-600 focus:outline-none focus:ring-4 focus:ring-gray-300 dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800">Batal</a>
                </div>
            </div>
        </form>
    </div>
