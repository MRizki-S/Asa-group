...
@section('content')
<div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6" x-data="kawasanManager({{ $allPembangunanKawasan->map(
        fn($p) => [
            'id' => $p->id,
            'nama' => $p->nama,
            'perumahan' => $p->perumahan->nama_perumahaan ?? '-',
        ...
        <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
            <div class="flex flex-col md:flex-row items-stretch md:items-center gap-4 flex-1">
                <div class="relative w-full md:w-80">
                    ...
                    <input type="text" x-model="searchQuery" @input="currentPage = 1" placeholder="Cari nama, perumahan, pengawas..." class="w-full text-gray-700 rounded-lg border border-gray-200 bg-white py-2.5 pl-10 pr-4 text-sm focus:border-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white transition-all" />
                </div>
            ...
            <div class="group rounded-lg border border-gray-200 bg-white p-5 shadow-sm hover:shadow-md transition-all dark:border-gray-700 dark:bg-gray-800">
                <div class="flex flex-col h-full justify-between">
                    ...
                    <div class="mt-5 pt-4 border-t border-gray-100 dark:border-gray-700">
                        @can('produksi.kawasan.pembangunan-kawasan.detail')
                        <a :href="'{{ route('produksi.pembangunanKawasan.index') }}/' + item.id" class="block w-full text-center py-2 text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors border border-blue-100 dark:bg-blue-900/20 dark:border-blue-800 dark:text-blue-400 shadow-sm">
                            Lihat Detail
                        </a>
                        @endcan
                    </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('kawasanManager', (initialData) => ({
        allData: initialData,
        searchQuery: '',
        filterStatus: 'proses',
        currentPage: 1,
        ...
        get filteredData() {
            return this.allData.filter(item => {
                ...
                const matchesSearch = item.nama.toLowerCase().includes(search) ||
                                      (item.perumahan && item.perumahan.toLowerCase().includes(search)) ||
