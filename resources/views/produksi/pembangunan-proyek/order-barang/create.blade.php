...
@section('pageActive', 'orderBarangProyek')
...
<div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6" x-data="createOrderProyekComponent()">

    <div x-data="{ pageName: 'Tambah Barang Keluar Proyek' }">
    ...
    <div class="mb-6 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 dark:border-gray-800 pb-3">
            <div>
                <h3 class="text-lg font-bold text-gray-800 dark:text-white/90">Tambah Barang Keluar Proyek</h3>
                <p class="text-xs text-gray-500">Pilih pembangunan proyek dan susun daftar barang ke dalam keranjang checkout order</p>
            </div>
            <a href="{{ route('produksi.pembangunanProyek.orderIndex') }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <select x-ref="proyekSelect"
                    ...
                    @foreach($pembangunanProyek as $pp)
                        <option value="{{ $pp->id }}">{{ $pp->label_formatted }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="md:col-span-3">
                ...
                <textarea x-model="catatan" rows="2" placeholder="Masukkan catatan atau pengingat order barang proyek jika ada..."
        ...
        <div class="lg:col-span-7 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-white/[0.03] flex flex-col h-[600px] lg:h-[680px]">
            ...
            <div class="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-5">
                <div class="pt-2">
                    ...
                    <!-- List Semua Barang Gudang (Card Grid 2 Kolom) -->
        ...
        <div class="hidden lg:flex lg:col-span-5 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-white/[0.03] flex-col h-[680px]">
            ...
            <div class="pt-3 border-t border-gray-100 dark:border-gray-800 mt-3 shrink-0 grid grid-cols-2 gap-2">
                ...
                <button type="button" @click="submitForm()"
                    :disabled="submitting || cart.length === 0 || !pembangunanProyekId"
                ...
                <div class="p-5 max-h-[60vh] overflow-y-auto space-y-4">
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-2 p-3 bg-blue-50/50 rounded-xl border border-blue-100 dark:bg-blue-950/20 dark:border-blue-900/40 text-xs">
                        <div>
                            ...
                            <span class="font-bold text-gray-800 dark:text-white ml-1" x-text="selectedProyekInfo?.label_formatted || '-'"></span>
                        </div>
                        <div>
...
<script>
function createOrderProyekComponent() {
    return {
        ...
        get selectedProyekInfo() {
            ...
            return this.allPembangunanProyek.find(pp => String(pp.id) === String(this.pembangunanProyekId)) || null;
        },
        ...
        init() {
            ...
            const preselectId = urlParams.get('pembangunan_proyek_id') || this.preselectProyekId;

            this.$nextTick(() => {
                let proyekSelectEl = $(this.$refs.proyekSelect);
                proyekSelectEl.empty().append('<option value="">-- Pilih Pembangunan Proyek --</option>');
                this.allPembangunanProyek.forEach(pp => {
                    proyekSelectEl.append(new Option(pp.label_formatted, pp.id, false, false));
                });

                proyekSelectEl.select2({
                    theme: 'bootstrap4',
                ...
                if (preselectId) {
                    this.pembangunanProyekId = preselectId;
                    proyekSelectEl.val(preselectId).trigger('change.select2');
                }
            });
        },
        ...
        processSubmitOrder() {
            ...
            let targetUrl = "{{ route('produksi.pembangunanProyek.orderStore') }}";
            let payload = {
            ...
            .then(async (res) => {
                ...
                if (res.ok) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Berhasil!',
                        text: data.message || 'Order barang proyek berhasil diajukan',
                        timer: 2000,
                        showConfirmButton: false
                    }).then(() => {
                        window.location.href = "{{ route('produksi.pembangunanProyek.orderIndex') }}";
                    });
                } else {
