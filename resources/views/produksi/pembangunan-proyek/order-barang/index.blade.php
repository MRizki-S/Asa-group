...
<div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6">
    <div x-data="{ pageName: 'Order Barang Proyek' }">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-gray-100 dark:border-gray-800 pb-4">
            <div>
                <h3 class="text-lg font-bold text-gray-800 dark:text-white">Daftar Order Barang Proyek</h3>
        ...
        <div class="mb-4 flex flex-wrap items-center gap-2">
            ...
            <a href="{{ route('produksi.pembangunanProyek.orderIndex', ['status' => 'diproses']) }}"
            ...
            <a href="{{ route('produksi.pembangunanProyek.orderIndex', ['status' => 'menunggu_spv']) }}"
            ...
            <a href="{{ route('produksi.pembangunanProyek.orderIndex', ['status' => 'selesai']) }}"
            ...
            <a href="{{ route('produksi.pembangunanProyek.orderIndex', ['status' => 'ditolak']) }}"
            ...
            <table class="w-full text-left text-sm text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse ($orders as $order)
                        @php
                            $proyek = $order->proyek;
                            $proyekNama = $proyek?->nama_project ?? $proyek?->nama ?? '-';
                            $pengawasNama = $proyek?->pengawas?->nama_lengkap ?? $proyek?->pengawas?->name ?? '-';
                            $statusMap = [
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                            ...
                            <td class="px-4 py-3">
                                @if ($order->pembangunan_proyek_id)
                                    <a href="{{ route('produksi.pembangunanProyek.show', $order->pembangunan_proyek_id) }}" class="group block">
                                        <div class="font-medium text-gray-900 group-hover:text-blue-600 dark:text-white dark:group-hover:text-blue-400 group-hover:underline">{{ $proyekNama }}</div>
                                    </a>
                                @else
                                    <div class="font-medium text-gray-900 dark:text-white">{{ $proyekNama }}</div>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-xs">
                                {{ $pengawasNama }}
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    @if ($order->status_order === 'selesai')
                                        <a href="{{ route('gudang.permintaanBarang.pembangunanProyek.notaPdf', $order->id) }}"
                                            target="_blank"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-8 text-center text-sm text-gray-500">
                                Belum ada data order barang proyek.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
