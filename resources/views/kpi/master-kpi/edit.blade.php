...
@section('content')
    <div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6">
        ...
        <form action="{{ route('kpi.komponen.update', $komponen->id) }}" method="POST" x-data="{ submitting: false }" @submit="if(submitting) { $event.preventDefault(); return; }; submitting = true">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 gap-6">
                ...
                <div class="flex flex-col-reverse md:flex-row justify-end gap-3 mt-4">
                    ...
                    <button type="submit" :disabled="submitting" :class="submitting ? 'opacity-50 cursor-not-allowed' : ''"
                        ...
                        <span x-text="submitting ? 'Memproses...' : 'Perbarui Master KPI'"></span>
                    </button>
                </div>
            </div>
        </form>
