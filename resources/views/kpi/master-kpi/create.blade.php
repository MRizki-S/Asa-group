...
@section('content')
    <div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6">
        <div x-data="{ pageName: 'Tambah Master KPI' }">
            @include('partials.breadcrumb')
        </div>

        <form action="{{ route('kpi.komponen.store') }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 gap-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="role_id"
                                ...
                                <option value="" class="bg-white dark:bg-gray-900 text-gray-700 dark:text-white">Pilih
                                    Role</option>
                        ...
                        <div>
                            ...
                            <select name="tipe_perhitungan"
                                ...
                                @foreach ($tipePerhitungan as $tipe)
                                    <option value="{{ $tipe }}"
                                        {{ old('tipe_perhitungan') == $tipe ? 'selected' : '' }}
                                        class="bg-white dark:bg-gray-900 text-gray-700 dark:text-white">
                                        {{ str_replace('_', ' ', $tipe) }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label class="mb-2.5 block text-sm font-medium text-gray-500 dark:text-gray-400">Label
                                Total</label>
                            <input type="text" name="label_total" value="{{ old('label_total', 'Total Task') }}"
                        ...
                        <div>
                            <label class="mb-2.5 block text-sm font-medium text-green-600 dark:text-green-500">Label
                                Tercapai</label>
                            <input type="text" name="label_tercapai" value="{{ old('label_tercapai', 'Tercapai') }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="label_tidak_tercapai"
                                value="{{ old('label_tidak_tercapai', 'Tidak Tercapai') }}"
                        ...
                        <table class="w-full text-left border-collapse" id="task-table">
                            <thead>
                                <tr class="border-b border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400">
                                    <th class="py-2 px-4 font-medium text-sm w-16">No</th>
                                    <th class="py-2 px-4 font-medium text-sm">Nama Task</th>
                                    <th class="py-2 px-4 font-medium text-sm w-20 text-center">Aksi</th>
                                </tr>
                            </thead>
                            <tbody id="task-container">
                                <tr
                                    ...
                                    <td class="py-4 px-4 text-sm text-gray-500 dark:text-gray-400 row-number">1</td>
                                    <td class="py-2 px-2">
                ...
                <div class="flex justify-end gap-3 mt-4">
                    <a href="{{ route('kpi.komponen.index') }}"
                        class="px-6 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 transition">
                        Batal
                    </a>
                    <button type="submit"
                        class="px-6 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 shadow-lg shadow-blue-600/20 transition">
                        Simpan Master KPI
                    </button>
                </div>
            </div>
