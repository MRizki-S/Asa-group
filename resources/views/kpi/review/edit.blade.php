...
<div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6">
    ...
    <form action="{{ route('kpi.review.update', $kpiUser->id) }}" method="POST" x-data="{ submitting: false }" @submit="if(submitting) { $event.preventDefault(); return; }; submitting = true">
        @csrf @method('PUT')
                            ...
                            <thead
                                ...
                                <tr>
                                    <th class="px-4 py-3 text-left text-[10px] font-bold text-gray-400 uppercase w-12 shrink-0">
                                        No</th>
                                    <th class="px-6 py-3 text-left text-[10px] font-bold text-gray-400 uppercase min-w-[320px]">Task
                                    </th>
                                    @if ($mode != 'select')
                                    <th
    ...
    {{ $komponen->komponen->tipe_perhitungan == 'DEVIASI_BUDGET' ? 'w-60' : ($mode == 'select' ? 'w-80' : 'w-40') }}">
                                    ...
                                    <th
                                        ...
                                        {{ $komponen->komponen->label_tidak_tercapai }}
                                    </th>
                                    @endif
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                ...
                                <tr class="bg-red-50/30 dark:bg-red-900/10">
                                    <td colspan="4" class="px-6 py-3">
                                        <p class="text-xs text-red-600 dark:text-red-400 italic leading-relaxed">
                                            <span class="font-semibold">Alasan:</span>
                                            {{ $task->alasan_tidak_tercapai }}
                                        </p>
                                    </td>
                                </tr>
                                @endif
        ...
        <div
            ...
            <div class="flex w-full flex-col-reverse md:flex-row justify-end gap-3">
                ...
                <button type="submit" :disabled="submitting" :class="submitting ? 'opacity-50 cursor-not-allowed' : ''"
                    ...
                    <span x-text="submitting ? 'Memproses...' : 'Simpan Hasil Review'"></span>
                </button>
                @endcan
            </div>
        </div>
