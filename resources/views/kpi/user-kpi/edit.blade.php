...
@section('content')
    <div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6" x-data="kpiCalculator()">
        <div x-data="{ pageName: 'Input Nilai KPI: {{ $kpiUser->user->nama_lengkap }}' }">
        ...
        <div class="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="rounded-2xl border border-gray-200 bg-blue-600 p-5 text-white shadow-lg shadow-blue-600/20">
                ...
                {{-- Menampilkan penjumlahan (Bobot * Skor) --}}
        ...
        <form action="{{ route('kpi.user.update', $kpiUser->id) }}" method="POST">
                                ...
                                <thead
                                    ...
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-left text-[10px] font-bold text-gray-400 uppercase">Jenis
                                            Data / Task</th>
                                        <th
                                            class="px-6 py-3 text-center text-[10px] font-bold text-gray-400 uppercase w-32">
                                            Target</th>
                                        <th
                                            class="px-6 py-3 text-center text-[10px] font-bold text-gray-400 uppercase w-32">
                                            Tercapai</th>
                                        ...
                                        <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01] transition-colors">
                                            ...
                                            <td class="px-6 py-3">
                                                <input type="number"
                                                    ...
                                                    x-model.number="values[{{ $task->id }}].target"
                                            ...
                                            <td class="px-6 py-3">
                                                <input type="number"
                                                    ...
                                                    x-model.number="values[{{ $task->id }}].tercapai"
                                                    class="w-full text-center bg-gray-50 dark:bg-gray-800 border-none rounded-lg text-sm font-bold text-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 py-1.5 transition">
                                            </td>
                                            <td class="px-6 py-4 text-center text-sm font-black text-gray-500 dark:text-gray-400"
                                                x-text="calculateRowPercent({{ $task->id }}) + '%'">
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
            ...
            {{-- Form Actions --}}
            <div
                class="mt-8 flex flex-col md:flex-row justify-between items-center gap-4 bg-white dark:bg-white/[0.03] p-6 rounded-2xl border border-gray-200 dark:border-gray-800 mb-10">
                <div class="flex items-center gap-4">
                    <label class="text-sm font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Simpan
                        sebagai:</label>
                    <select name="status"
                        class="rounded-xl border-gray-300 bg-white dark:border-gray-700 dark:bg-gray-800 text-gray-700 dark:text-white text-sm font-bold focus:ring-blue-500">
                        <option value="draft" {{ $kpiUser->status == 'draft' ? 'selected' : '' }}>DRAFT (Bisa Diedit)
                        </option>
                        <option value="final" {{ $kpiUser->status == 'final' ? 'selected' : '' }}>FINAL (Kunci Nilai)
                        </option>
                    </select>
                </div>
                <div class="flex gap-3">
                    <a href="{{ route('kpi.user.index') }}"
                        class="px-8 py-3 text-sm font-bold text-gray-500 bg-gray-100 rounded-xl hover:bg-gray-200 transition uppercase tracking-widest">Batal</a>
                    <button type="submit"
    ...
    <script>
        function kpiCalculator() {
            return {
                // Konfigurasi Bobot per Komponen
                komponenData: {
                    @foreach ($kpiUser->details as $komponen)
                        {{ $komponen->id }}: {
                ...
                calculateRowPercent(id) {
                    let t = this.values[id].target;
                    let a = this.values[id].tercapai;
                    if (t <= 0) return 0;
                ...
                calculateKepatuhan(komponenId) {
                    ...
                    return Math.min(Math.round((totalA / totalT) * 100), 100);
                },
                ...
                // Menghitung Nilai Akhir Komponen: (Bobot/100) * Skor
                getWeightedNilai(komponenId, bobot) {
                    let skor = this.getSkor(komponenId);
                    return (bobot / 100) * skor;
                },

                // Menghitung Total Skor Akhir (Sum of Weighted Nilai)
                get grandTotalSkor() {
                    let totalNilai = 0;
