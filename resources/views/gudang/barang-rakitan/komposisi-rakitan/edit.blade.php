...
<div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6">
    ...
    <form action="{{ $formAction ?? route('gudang.komposisiRakitan.store') }}" method="POST"
        x-data="komposisiRakitanForm({{ Js::from($masterBarangs) }}, {{ Js::from($initialData ?? null) }})">
        @csrf
        @if (!empty($formMethod))
        @method($formMethod)
        @endif
...
<script>
    function komposisiRakitanForm(masterBarangs, initialData = null) {
        const findBarangOption = (id) => masterBarangs.find(barang => String(barang.id) === String(id));
        const getSatuanList = (barangId) => findBarangOption(barangId)?.satuans ?? [];

        return {
            masterBarangs,
            satuanHasilList: [],
            barangHasilId: initialData?.barang_hasil_id ?? '',
            satuanHasilId: initialData?.satuan_hasil_id ?? '',
            qtyHasil: initialData?.qty_hasil ?? '',
            qtyHasilBase: '',
            qtyHasilBaseDisplay: '',
            hasilKonversi: 1,
            ...
            items: initialData?.items?.length ? initialData.items.map((item, index) => ({
                key: Date.now() + index,
                barang_bahan_id: item.barang_bahan_id,
                satuan_id: item.satuan_id,
                satuanList: getSatuanList(item.barang_bahan_id),
                qty: item.qty,
                qty_base: '',
                konversi: 1,
            ...
            syncSelect2Value(selectEl, value) {
            ...
            applyInitialBarangHasil() {
                if (!this.barangHasilId) return;

                $(this.$refs.barangHasilSelect).val(this.barangHasilId).trigger('change.select2');
                this.applyBarangHasil(this.barangHasilId, this.satuanHasilId);
            },
                ...
                $(selectEl).select2({
                    ...
                    this.$nextTick(() => {
                        ...
                        if (defaultSatuan && item._satuanSelectEl) {
                            ...
                            this.syncSelect2Value(item._satuanSelectEl, defaultSatuan.id);
                        } else if (item._satuanSelectEl) {
                            this.syncSelect2Value(item._satuanSelectEl, '');
                        }
                        this.hitungItemBase(index);
                    });
                });

                if (this.items[index].barang_bahan_id) {
                    this.syncSelect2Value(selectEl, this.items[index].barang_bahan_id);
                    this.applyBarangBahan(index, this.items[index].barang_bahan_id, this.items[index].satuan_id);
                }
            },
            ...
            initSatuanBahanSelect(selectEl, index) {
                ...
                if (this.items[index].satuan_id) {
                    ...
                    item.konversi = Number(satuan?.konversi_ke_base || 1);
                    this.syncSelect2Value(selectEl, item.satuan_id);
                    this.hitungItemBase(index);
                }
            },

            applyBarangBahan(index, barangId, selectedSatuanId = null) {
                const item = this.items[index];
                item.barang_bahan_id = barangId;
                const barang = this.findBarang(item.barang_bahan_id);
                ...
                this.$nextTick(() => {
                    const satuanTerpilih = selectedSatuanId
                        ? item.satuanList.find(satuan => String(satuan.id) === String(selectedSatuanId))
                        : null;
                    const defaultSatuan = satuanTerpilih || item.satuanList.find(satuan => satuan.is_default) || item.satuanList[0];
