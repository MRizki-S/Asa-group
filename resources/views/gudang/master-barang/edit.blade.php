...
<div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6">
    ...
    <form action="{{ route('gudang.masterBarang.update', $barangEdit->kode_barang) }}" method="POST">
        @csrf
        @method('PUT')

        <!-- input table master Barang -->
        <div x-data="{ is_stock: '{{ old('is_stock', $barangEdit->is_stock ? '1' : '0') }}' }" class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] mb-6">
            <div class="px-5 py-4 sm:px-6 sm:py-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="kode_barang" name="kode_barang" value="{{ $barangEdit->kode_barang }}"
                            placeholder="BRG-XXXX"
                            class="w-full bg-gray-100 border text-gray-500 text-sm rounded-lg p-2.5
                 dark:bg-gray-700 dark:text-gray-400 border-gray-300 cursor-not-allowed" readonly>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="satuan_id" id="satuanSelect" required
                            ...
                            <option value="{{ $item->id }}" {{ old('satuan_id', $barangEdit->base_unit_id) == $item->id ? 'selected' : '' }}>
                                {{ $item->nama }}
                            </option>
                            @endforeach
                        </select>
                ...
                <div x-show="is_stock === '1'" x-transition class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 pt-4 border-t border-gray-100 dark:border-gray-800">
                    ...
                    <div>
                        ...
                        <input type="number" id="minimal_stock_ubs" name="minimal_stock_ubs" step="0.01" min="0"
                            x-bind:required="is_stock === '1'" value="{{ old('minimal_stock_ubs', $minimal_stock_ubs ? rtrim(rtrim($minimal_stock_ubs, '0'), '.') : 0) }}"
                            class="w-full bg-gray-50 border text-gray-900 text-sm rounded-lg p-2.5 dark:bg-gray-800 dark:text-white border-gray-300">
                    </div>
                </div>
            </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] mb-6">
            <div class="px-5 py-4">
                ...
                <div x-data="satuanKonversi({{ Js::from($satuan) }}, {{ Js::from($barangEdit->satuanKonversi) }})" class="overflow-x-auto">
                                            ...
                                            <select :name="`items[${index}][satuan_id]`"
                                                ...
                                                x-init="
                                                        $nextTick(() => {
                                                            ...
                                                            if (item.satuan_id) {
                                                                $select.val(item.satuan_id).trigger('change.select2');
                                                            }
...
<script>
    function satuanKonversi(satuanList, existingKonversi = []) {
        
        let initialItems = [];
        if (existingKonversi && existingKonversi.length > 0) {
            initialItems = existingKonversi.map(item => ({
                id: item.id || Date.now() + Math.random(),
                satuan_id: item.satuan_id,
                konversi: parseFloat(item.konversi_ke_base),
