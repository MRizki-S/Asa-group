...
@section('content')
    <div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6" x-data="pengajuanUpahForm()" x-init="init()">
        ...
        <div x-show="currentStep === 1" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-white/[0.03] mb-5">
                <div class="px-5 py-4 sm:px-6 border-b border-gray-100 dark:border-gray-700">
                    <h3 class="text-base font-semibold text-gray-800 dark:text-white">Informasi Pengajuan {{ $isAbm ? 'ABM' : 'Mangoon' }}</h3>
                </div>
                <div class="px-5 py-5 sm:px-6 grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <div class="relative"
                            x-data="{ tampil: '{{ now()->format('d-m-Y') }}', simpan: '{{ now()->format('Y-m-d') }}' }">
        ...
        <div x-show="currentStep === 2" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-white/[0.03] mb-5">
                <div class="px-5 py-4 sm:px-6 border-b border-gray-100 dark:border-gray-700">
                    <h3 class="text-base font-semibold text-gray-800 dark:text-white">Ringkasan Pengajuan</h3>
                </div>
    ...
    <script>
        ...
        function pengajuanUpahForm() {
            return {
                ...
                // --- Step 2 ---
                tukangDetails: [],   // array of { id, kode, nama_tukang, gaji_harian_default, jam_kerja_default, details: [{tanggal, hadir, nominal, jam_normal, alokasi_normal:[], alokasi_lembur:[]}] }
                openAccordion: 0,
                ...
                getReferensiOptions(jenis) {
                    ...
                    if (jenis === 'pembangunan_proyek') return this.pembangunanProyeks;
                    return [];
                },
                ...
                getReferensiLabel(jenis, id) {
                    let list = [];
                    if (jenis === 'pembangunan_unit') list = this.pembangunanUnits;
                    else if (jenis === 'pembangunan_kawasan') list = this.pembangunanKawasans;
                    else if (jenis === 'pembangunan_proyek') list = this.pembangunanProyeks;
                ...
                submitForm(statusVal) {
                    ...
                    form.method = 'POST';
                    form.action = @json($storeDraftRoute);
