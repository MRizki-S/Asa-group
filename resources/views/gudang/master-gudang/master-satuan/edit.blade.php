...
@section('pageActive', 'MasterSatuan')
...
@section('content')
    ...
    <div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6">
        ...
        <!-- Breadcrumb -->
        <div x-data="{ pageName: 'MasterSatuan' }">
        ...
        <form action="{{ route('gudang.masterSatuanBarang.update', $satuan->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] mb-6">
                <div class="px-5 py-4 sm:px-6 sm:py-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <!-- Nama Barang -->
                        <div>
                            <label for="nama_satuan" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                Nama Satuan <span class="text-red-500">*</span>
                            </label>
                            <input type="text" id="nama_satuan" name="nama_satuan" required
                                value="{{ old('nama_satuan', $satuan->nama) }}" placeholder="Contoh: sak,pcs,dus"
                                class="w-full bg-gray-50 border text-gray-900 text-sm rounded-lg p-2.5
                            dark:bg-gray-700 dark:text-white
                            @error('nama_satuan') border-red-500 @else border-gray-300 @enderror">
                            @error('nama_satuan')
            ...
            <div class="flex justify-end gap-2">
                <button type="button" onclick="window.location.replace('{{ route('gudang.masterSatuanBarang.index') }}')"
