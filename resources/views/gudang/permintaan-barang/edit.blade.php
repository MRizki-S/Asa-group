    ...
    <div class="mb-6 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 dark:border-gray-800 pb-3">
            <div>
                ...
                <p class="text-xs text-gray-500">Edit daftar barang permintaan yang ditolak sebelum diajukan kembali</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="grid grid-cols-2 gap-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="text"
                            ...
                            x-init="flatpickr($el, {
                                dateFormat: 'd-m-Y',
                                defaultDate: '{{ $order->tanggal_diajukan ? \Carbon\Carbon::parse($order->tanggal_diajukan)->format('d-m-Y') : now()->format('d-m-Y') }}',
            ...
            @if($category === 'pembangunan_unit')
                <div>
                    ...
                    <select x-ref="pembangunanSelect" disabled
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase mb-2">QC Pembangunan Unit <span class="text-xs text-gray-400 font-normal">(Diset)</span></label>
                    <select x-ref="qcSelect" disabled
                        class="w-full rounded-xl border-gray-300 bg-gray-100 p-3 text-sm text-gray-800 dark:bg-gray-700 dark:border-gray-600 dark:text-white opacity-75 cursor-not-allowed">
            ...
            @elseif($category === 'pembangunan_kawasan')
                <div>
                    ...
                    <select x-ref="kawasanSelect" disabled
                        class="w-full rounded-xl border-gray-300 bg-gray-100 p-3 text-sm text-gray-800 dark:bg-gray-700 dark:border-gray-600 dark:text-white opacity-75 cursor-not-allowed">
...
function createOrderUnitComponent() {
    return {
        orderId: {{ $order->id }},
        category: '{{ $category }}',
        tanggalNbkTampil: '{{ $order->tanggal_diajukan ? \Carbon\Carbon::parse($order->tanggal_diajukan)->format('d-m-Y') : now()->format('d-m-Y') }}',
        tanggalNbkSimpan: '{{ $order->tanggal_diajukan ? \Carbon\Carbon::parse($order->tanggal_diajukan)->format('Y-m-d') : now()->format('Y-m-d') }}',
        waktuLive: '{{ now()->format('H:i:s') }}',
        clockInterval: null,
        pembangunanUnitId: '{{ $order->pembangunan_unit_id ?? '' }}',
        pembangunanKawasanId: '{{ $order->pembangunan_kawasan_id ?? '' }}',
        pembangunanProyekId: '{{ $order->pembangunan_proyek_id ?? '' }}',
        activePeriodeLabel: '',
        kawasanHasActivePeriode: null,
        qcId: '{{ $order->pembangunan_unit_qc_id ?? '' }}',
        catatan: {!! json_encode($order->catatan ?? '') !!},
        jenisOrderType: '{{ $order->jenis_order ?? "stock" }}',
        qcs: [],
        rapItems: [],
        cart: [],
        initialDetails: {!! $order->details->map(function($d) {
            $satuans = collect();
            if ($d->barang && $d->barang->baseUnit) {
                $satuans->push([
                    'id' => $d->barang->base_unit_id,
                    'nama_satuan' => $d->barang->baseUnit->nama,
                    'konversi_ke_base' => 1
                ]);
            }
            if ($d->barang && $d->barang->satuanKonversi) {
                foreach ($d->barang->satuanKonversi as $sk) {
                    if ($sk->satuan) {
                        $satuans->push([
                            'id' => $sk->satuan_id,
            ...
            // Hitung total_ordered_base dari RAP (exclude order ini sendiri)
            $totalOrderedBase = 0;
            $volumeRap = 0;
            $faktorKonversiRap = 1;
            if ($d->rap_bahan_id && $d->rapBahan) {
                $rap = $d->rapBahan;
                $faktorKonversiRap = (float)($rap->faktor_konversi ?? 1);
                $volumeRap = (float)$rap->jumlah_standar * $faktorKonversiRap;
            ...
            return [
                ...
                'qty' => (float)$d->jumlah_input,
                'stok_gudang_base' => (float)($d->barang && $d->barang->stock ? $d->barang->stock->sum('jumlah_stock') : 0),
                'base_unit_id' => $d->barang->base_unit_id ?? null,
                'base_unit_nama' => $d->barang->baseUnit->nama ?? '',
                'is_rap' => (bool)$d->rap_bahan_id,
                'rap_id' => $d->rap_bahan_id,
                'volume_rap' => $volumeRap,
                'faktor_konversi_rap' => $faktorKonversiRap,
                'total_ordered_base' => $totalOrderedBase,
                'alasan' => $d->alasan_permintaan_tidak_sesuai_rap ?? '',
                'satuans' => $satuans->unique('id')->values()->toArray(),
                'is_stock' => (bool) ($d->barang->is_stock ?? true)
            ];
        })->toJson() !!},
        ...
        async init() {
            ...
            // Load detail barang ke keranjang checkout
            this.cart = JSON.parse(JSON.stringify(this.initialDetails));

            // Untuk kawasan/proyek: sinkronkan stok_gudang_base dari allBarangGudang
            if (this.category !== 'pembangunan_unit') {
                this.cart = this.cart.map((item) => {
                    let barangData = this.allBarangGudang.find((b) => b.id == item.barang_id);
                    if (barangData) {
                        item.stok_gudang_base = barangData.stok_gudang || 0;
                        item.satuans = barangData.satuans && barangData.satuans.length ? barangData.satuans : item.satuans;
                    }
                    return item;
                });
            }
            ...
            this.$nextTick(async () => {
                if (this.category === 'pembangunan_unit') {
                    ...
                    if (this.pembangunanUnitId) {
                        $(this.$refs.pembangunanSelect).val(this.pembangunanUnitId).trigger('change.select2');
                        await this.loadQcList();
                    }
                    ...
                    if (this.qcId) {
                        $(this.$refs.qcSelect).val(this.qcId).trigger('change.select2');
                    }
                    ...
                    if (this.pembangunanKawasanId) {
                        $(this.$refs.kawasanSelect).val(this.pembangunanKawasanId).trigger('change.select2');
                    ...
                    if (this.pembangunanProyekId) {
                        $(this.$refs.proyekSelect).val(this.pembangunanProyekId).trigger('change.select2');
                    }
                }
            });
        },
        ...
        // Versi loadQcList tanpa reset cart — dipakai saat init halaman edit
        async loadQcList() {
            if (!this.pembangunanUnitId) return;
            this.loadingQc = true;
            try {
                ...
                if (res.ok) {
                    ...
                    // Set rapItems dari qcId yang sudah dipilih tanpa mereset cart
                    if (this.qcId) {
                    ...
                    // Sinkronkan nilai stok & RAP dari sumber data API ke item yang sudah ada di cart
                    // Kumpulkan semua rap_bahan dari semua QC
                    let allRapBahan = [];
                    this.qcs.forEach((q) => {
                        if (q.rap_bahan) allRapBahan = allRapBahan.concat(q.rap_bahan);
                    });

                    this.cart = this.cart.map((item) => {
                        if (item.is_rap && item.rap_id) {
                            // Cari data RAP dari API berdasarkan rap_id
                            let rapData = allRapBahan.find((r) => r.id == item.rap_id);
                            if (rapData) {
                                item.stok_gudang_base = rapData.stok_gudang || 0;
                                item.total_ordered_base = rapData.total_ordered_base || 0;
                                item.volume_rap = rapData.volume || 0;
                                item.faktor_konversi_rap = rapData.faktor_konversi || 1;
                                item.satuans = rapData.satuans && rapData.satuans.length ? rapData.satuans : item.satuans;
                            }
                        } else if (!item.is_rap) {
                            // Sinkronkan stok dari allBarangGudang untuk barang luar RAP
                            let barangData = this.allBarangGudang.find((b) => b.id == item.barang_id);
                            if (barangData) {
                                item.stok_gudang_base = barangData.stok_gudang || 0;
                                item.satuans = barangData.satuans && barangData.satuans.length ? barangData.satuans : item.satuans;
        ...
        processSubmitOrder() {
            ...
            let targetUrl = "{{ route('gudang.permintaanBarang.update', ['id' => '___ID___']) }}".replace('___ID___', this.orderId) + "?category=" + this.category;
            let payload = {};
            ...
            } else if (this.category === 'pembangunan_kawasan' || this.category === 'pembangunan_proyek_mangoon') {
                payload = {
                    catatan: this.catatan,
                    jenis_order: this.jenisOrderType,
            ...
            .then(async (res) => {
                ...
                if (res.ok) {
                    Swal.fire({
                        ...
                        text: data.message || 'Perubahan berhasil disimpan. Klik tombol Ajukan Ulang di halaman detail untuk mengajukan kembali.',
                        confirmButtonText: 'Ke Halaman Detail',
                        confirmButtonColor: '#3b82f6',
                    }).then(() => {
                        window.location.href = "{{ route('gudang.permintaanBarang.show', ['id' => '___ID___']) }}".replace('___ID___', this.orderId) + "?jenis_order=" + this.category;
                    });
                } else {
            ...
            .catch((err) => {
                this.submitting = false;
                this.showConfirmModal = false;
                Swal.fire('Error', 'Terjadi kesalahan server saat memperbarui order', 'error');
            });
        }
    };
}
