...
<div class="mx-auto max-w-[--breakpoint-2xl] p-4 md:p-6" x-data="createOrderUnitComponent()">
    ...
    <div x-show="showMobileCart" class="fixed inset-0 z-[99999] lg:hidden" x-cloak>
        ...
        <div class="fixed inset-x-0 bottom-0 max-h-[85vh] bg-white dark:bg-gray-900 rounded-t-3xl shadow-2xl flex flex-col overflow-hidden"
            x-show="showMobileCart"
            x-transition:enter="transition ease-out duration-300 transform"
                        ...
                        <div class="grid grid-cols-2 gap-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                            <div>
                                ...
                                <div class="flex items-center rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 overflow-hidden">
                                    <button type="button" @click="decrementQty(item)" class="px-3 py-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-300 transition-colors select-none shrink-0">
                                    ...
                                    <input type="text" inputmode="decimal" x-model="item.qty" @change="validateQtyOnChange(item)" placeholder="0" class="w-full text-base font-bold text-center bg-transparent text-gray-800 dark:text-white border-0 focus:ring-0 focus:outline-none p-1">
                                    <button type="button" @click="incrementQty(item)" class="px-3 py-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-300 transition-colors select-none shrink-0">
                            ...
                            <div>
                                <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">Satuan</label>
                                <select x-model="item.satuan_id" class="w-full text-base p-2 rounded-xl border-gray-300 bg-white text-gray-800 dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-blue-500">
                                    <template x-for="s in item.satuans" :key="s.id">
                                        <option :value="s.id" class="text-gray-800 dark:text-white" x-text="s.nama_satuan"></option>
                                    </template>
                                </select>
            ...
            <!-- Drawer Footer -->
            <div class="p-5 border-t border-gray-100 dark:border-gray-800 shrink-0 grid grid-cols-2 gap-3 bg-gray-50/50 dark:bg-gray-800/50">
                ...
                <button type="button" @click="showMobileCart = false; submitForm();"
                    :disabled="submitting || cart.length === 0"
                    class="w-full py-3 px-4 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-xs font-bold rounded-xl transition-all shadow-md flex items-center justify-center gap-1.5">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
