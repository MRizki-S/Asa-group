...
<head>
    <meta charset="utf-8">
    <title>Bulk Nota Barang Keluar</title>
    <style>
        ...
        .info-table td { padding: 1.5px 3px; font-size: 7.5pt; border: none; vertical-align: top; }
...
@php
    ...
    foreach ($orders as $order) {
        // Kunci 8 item per setengah nota agar muat presisi dalam 139mm tanpa meluber ke halaman baru
        $chunks = $order->details->chunk(8)->values();
        $totalChunks = $chunks->count();

        foreach ($chunks as $cIdx => $chunkItems) {
            $allSlots[] = [
                'order' => $order,
                'part' => $cIdx + 1,
        ...
        <tbody>
            ...
            <tr class="row-top">
                <td>
                    @php
                        $order = $topSlot['order'];
                        $tglOrder = $order->tanggal_diajukan ? \Carbon\Carbon::parse($order->tanggal_diajukan)->format('d/m/Y') : date('d/m/Y');
                        $jamOrder = $order->tanggal_diajukan ? \Carbon\Carbon::parse($order->tanggal_diajukan)->format('H:i') : date('H:i');
                        ...
                        if ($category === 'pembangunan_unit') {
                            ...
                            $namaBlok = $unit?->blok?->nama_blok ?? '-';
                            $namaUnit = $unit?->nama_unit ?? '-';
                            $pengawas = $order->pembangunanUnit?->pengawas?->nama_lengkap ?? $order->pembangunanUnit?->pengawas?->name ?? '-';
                            $qcNama = $order->qc?->nama_qc ?? '-';
                            $subTitle = 'ABM Group — Pembangunan Unit • ' . $namaPerumahan;
                            ...
                            $periodeLabel = $periode
                                ? (($periode->tanggal_mulai ? \Carbon\Carbon::parse($periode->tanggal_mulai)->format('d/m/Y') : '-')
                                   . ' s/d '
                                   . ($periode->tanggal_selesai ? \Carbon\Carbon::parse($periode->tanggal_selesai)->format('d/m/Y') : 'Sekarang'))
                                : '-';
                            $subTitle = 'ABM Group — Pembangunan Kawasan • ' . $namaPerumahan;
                        } else {
                            $proyek = $order->proyek;
                            ...
                            $subTitle = 'ABM Group / Mangoon — Pembangunan Proyek';
                        }
                    @endphp
                    ...
                    <table class="header-wrap">
                        <tr>
                            ...
                            <td style="width: 35%;" class="nbk-box">
                                ...
                                @if($topSlot['totalParts'] > 1)
                                    <div class="badge-part">[Bagian {{ $topSlot['part'] }} dari {{ $topSlot['totalParts'] }}]</div>
                                @endif
                            </td>
                        </tr>
                    </table>
                    ...
                    <table class="info-table">
                        @if($category === 'pembangunan_unit')
                            <tr>
                                ...
                                <td class="info-value">{{ $namaPerumahan }} &mdash; {{ $namaTahap }}</td>
                                <td class="info-label">Tanggal</td>
                                <td class="info-sep">:</td>
                                <td class="info-value">{{ $tglOrder }}</td>
                            </tr>
                            <tr>
                                <td class="info-label">Rumah Blok / No.</td>
                                <td class="info-sep">:</td>
                                <td class="info-value">{{ $namaBlok }} / {{ $namaUnit }}</td>
                                <td class="info-label">Jam</td>
                                <td class="info-sep">:</td>
                                <td class="info-value">{{ $jamOrder }} WIB</td>
                            </tr>
                            <tr>
                                <td class="info-label">QC RAP</td>
                                <td class="info-sep">:</td>
                                <td class="info-value">{{ $qcNama }}</td>
                        ...
                        @elseif($category === 'pembangunan_kawasan')
                            <tr>
                                <td class="info-label">Nama Proyek</td>
                                <td class="info-sep">:</td>
                                <td class="info-value">{{ $namaPerumahan }}</td>
                                <td class="info-label">Tanggal</td>
                                <td class="info-sep">:</td>
                                <td class="info-value">{{ $tglOrder }}</td>
                            </tr>
                            <tr>
                                <td class="info-label">Nama Kawasan</td>
                                <td class="info-sep">:</td>
                                <td class="info-value">{{ $namaKawasan }}</td>
                                <td class="info-label">Jam</td>
                                <td class="info-sep">:</td>
                                <td class="info-value">{{ $jamOrder }} WIB</td>
                            </tr>
                            <tr>
                                ...
                                <td class="info-sep">:</td>
                                <td class="info-value">{{ $periodeLabel }}</td>
                                <td class="info-label">Mandor</td>
                                <td class="info-sep">:</td>
                                <td class="info-value">{{ $pengawas }}</td>
                            </tr>
                    ...
                    <table class="sig-table">
                        <tr>
                            <td>
                                ...
                                <div style="font-size: 7.5pt; color: #555;">{{ $category === 'pembangunan_kawasan' ? 'Pengawas Kawasan' : 'Pengawas Proyek' }}</div>
                                <div class="sig-space"></div>
                                <div><span class="sig-name">({{ $pengawas }})</span></div>
                                <div style="font-size: 7pt; color: #888;">Nama Jelas &amp; Ttd</div>
                            </td>
                            <td>
                                <div>Diserahkan Oleh,</div>
                                <div style="font-size: 7.5pt; color: #555;">Staf Gudang</div>
                                <div class="sig-space"></div>
                        ...
                        <table class="header-wrap">
                            <tr>
                                ...
                                <td style="width: 35%;" class="nbk-box">
                                    <div class="nbk-number">{{ $order->nomor_nbk ?? $order->nomor_order }}</div>
                                    <div class="nbk-ref">Ref: {{ $order->nomor_order }}</div>
                                    @if($botSlot['totalParts'] > 1)
                                        <div class="badge-part">[Bagian {{ $botSlot['part'] }} dari {{ $botSlot['totalParts'] }}]</div>
                                    @endif
                                </td>
                            </tr>
                        </table>
