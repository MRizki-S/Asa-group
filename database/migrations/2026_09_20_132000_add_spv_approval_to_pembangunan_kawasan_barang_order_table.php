...
return new class extends Migration
{
    ...
    {
        // 1. Tambah 'menunggu_spv' ke enum status_order pada pembangunan_kawasan_barang_order
        DB::statement("ALTER TABLE `pembangunan_kawasan_barang_order` MODIFY COLUMN `status_order` ENUM('diproses', 'menunggu_spv', 'selesai', 'ditolak', 'pengembalian') NOT NULL DEFAULT 'diproses'");

        // 2. Tambah kolom approval SPV dan NBK pada pembangunan_kawasan_barang_order
        Schema::table('pembangunan_kawasan_barang_order', function (Blueprint $table) {
            if (!Schema::hasColumn('pembangunan_kawasan_barang_order', 'nomor_nbk')) {
                $table->string('nomor_nbk', 50)->nullable()->after('nomor_order');
            }
            if (!Schema::hasColumn('pembangunan_kawasan_barang_order', 'gudang_by')) {
                $table->foreignId('gudang_by')->nullable()->after('status_order')->constrained('users')->nullOnDelete();
            }
            if (!Schema::hasColumn('pembangunan_kawasan_barang_order', 'tanggal_gudang')) {
                $table->dateTime('tanggal_gudang')->nullable()->after('gudang_by');
            }
            if (!Schema::hasColumn('pembangunan_kawasan_barang_order', 'catatan_gudang')) {
                $table->text('catatan_gudang')->nullable()->after('tanggal_gudang');
            }
            if (!Schema::hasColumn('pembangunan_kawasan_barang_order', 'spv_by')) {
                $table->foreignId('spv_by')->nullable()->after('catatan_gudang')->constrained('users')->nullOnDelete();
            }
            if (!Schema::hasColumn('pembangunan_kawasan_barang_order', 'tanggal_spv')) {
                $table->dateTime('tanggal_spv')->nullable()->after('spv_by');
            }
            if (!Schema::hasColumn('pembangunan_kawasan_barang_order', 'acc_by')) {
        ...
        Schema::table('pembangunan_kawasan_barang_order_detail', function (Blueprint $table) {
            if (!Schema::hasColumn('pembangunan_kawasan_barang_order_detail', 'jumlah_acc')) {
                $table->decimal('jumlah_acc', 12, 3)->nullable()->after('jumlah_input');
            }
            if (!Schema::hasColumn('pembangunan_kawasan_barang_order_detail', 'jumlah_acc_base')) {
    ...
    public function down(): void
    {
        ...
        DB::statement("ALTER TABLE `pembangunan_kawasan_barang_order` MODIFY COLUMN `status_order` ENUM('diproses', 'selesai', 'ditolak', 'pengembalian') NOT NULL DEFAULT 'diproses'");
    }
};
